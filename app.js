...
// json 객체 데이터 배열을 forEach 메서드로 순회 - 배열 요소를 각각을 분리 - 분리된 각각의 데이터는 item 파라미터에 전달
...
const closeBtn = document.querySelector('.ri-close-line'); // x버튼 요소 저장
...
const activateLightBox = (thisElmt) => {
  ...
  const thisElmtDataFilter = thisElmt.getAttribute('data-filter'); // 클릭한 이미지의 data -filter 속성 값 저장
  ...
  const cateElmt = document.querySelector('.title p'); // 카테고리 문자 요소(p 태그) 저장
  ...
  // 라이트 박수가 활성화 될 때 클릭된 이미지 요소의 src 속성 값을 읽어 라이트 박스 이미지 src 속성값으로 전달
...
function showItems(thisItem) {
  ...
  if (thisCategory === 'all') { // 클릭한 요소의 속성값이 all일 때
    showAndHide(images, true); // images array add show remove hide
  }

  // 클릭한 요소의 data-filter 속성값과 이미지 요소의 data-filter 속성값이 일치하는 것만 필터링 
...
closeBtn.addEventListener('click', closeLightBox); // x 버튼을 클릭 했을 때 함수 실행
overlay.addEventListener('click', closeLightBox); // 배경 오버레이를 클릭했을 때 함수 실행

const spanAttr = document.querySelector('span').getAttribute('class');

//get attribute() : https://urliveblogger.blogspot.com/2021/01/js-getAttribute.html
//set attribute() : https://urliveblogger.blogspot.com/2021/01/js-setAttribute.html
//filter() : https://velog.io/@haleyjun/JavaScript-filter-%EC%82%AC%EC%9A%A9%EB%B2%95
// method : 어떠한 객체의 멤버가 함수일 경우 메서드라고 부른다.
// Array.from() : 유사배열 (NodeList, HTMLCollection)을 정식 배열로 전환
// setTimeout() : freecodecamp.org/korean/news/how-to-set-a-timer-in-javascript/
// 첫 번째 파라미터 : 함수 - 실행할 코드 , 두번째 파라미터 : 숫자 - 시간 (밀리초 단위) == 밀리초 단위 시간 이후 함수 실행
// setTimeout(function () {
//   alert('hello world');
// }, 1000);
// setInterval() :  https://ko.javascript.info/settimeout-setinterval
// 2초 간격으로 메시지를 보여줌
// 변수의 값을 저장하게 되면, 변수를 이용하여 일정 시간 이후 실행을 정지시킬 수 있다. (clearInterval(timerID))
...
// let result = arr.filter((value) => value === 4); // =이 3개일 때 더 엄격함. 문자 타입 모두 똑같아야만 뽑아올 수 있음. 2개 일시에는 문자만 같으면 모두 뽑아옴.
// console.log(result);
// const imgAttr = document.querySelector('#img');
...
// const result =
//   thisElmtDataFilter.charAt(0).toUpperCase() + thisElmtDataFilter.slice(1);
